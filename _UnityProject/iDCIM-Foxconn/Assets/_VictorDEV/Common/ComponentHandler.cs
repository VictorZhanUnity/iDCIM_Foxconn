...
namespace VictorDev.ComponentUtils
{
    public abstract class ComponentHandler
    {
        ...
        /// <summary>
        /// 持續往每個上層父物件尋找指定類別T，直到找到為止
        /// </summary>
        public static T FindTargetInParents<T>(Transform parentOfTarget) where T : Component
        {
            // 循环检查父级对象，直到找到 Canvas 或者父级对象为空
            while (parentOfTarget != null)
            {
                if (parentOfTarget.TryGetComponent<T>(out T result)) return result;

                // 往上一层查找
                parentOfTarget = parentOfTarget.parent;
            }

            // 如果循环结束仍未找到 Canvas，则返回 null
            return null;
        }
        ...
        /// 取得物件高度(需有Collider)
        /// </summary>
        public static float GetMeshHeight(MeshFilter meshFilter)
        {
            if (meshFilter != null)
            {
                // 獲取Mesh的Bounds
            ...
            {
                Debug.LogError("這個物件沒有MeshFilter組件！");
                return 0;
            }
        }
    }
